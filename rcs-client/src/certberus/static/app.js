...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadCurrentView() {
        ...
        if (state.activeView === 'dashboard') {
            ...
            await renderLogs();
        } else if (state.activeView === 'federation') {
    ...
    async function renderCertificates() {
        ...
        const tableRows = certs.map(cert => `
            <tr>
                ...
                <td>
                    <span class="status-badge ${cert.revoked_at ? 'revoked' : 'active'}" ${cert.revoked_at && cert.revoke_reason ? `title="Razón de revocación: ${cert.revoke_reason}" style="cursor: help;"` : ''}>
                        ${cert.revoked_at ? 'Revocado' : 'Activo'}
                        ${cert.revoked_at && cert.revoke_reason ? '<i class="fas fa-info-circle" style="margin-left: 4px; opacity: 0.8;"></i>' : ''}
                    </span>
                </td>
                <td>
                    ${!cert.revoked_at ? `<button class="action-btn" onclick="openRevokeModal('${cert.serial_number}')"><i class="fas fa-ban"></i> Revocar</button>` : '<span style="color: var(--text-secondary); font-size: 0.8rem;">Revocado</span>'}
                </td>
            </tr>
        `).join('');
    ...
    async function renderHierarchy() {
        ...
        let cards = cas.map(ca => {
            const isLevel3 = ca.parent_id !== null;
            const levelLabel = isLevel3 ? 'Level 3 Sub-CA' : 'Level 2 Intermediate';
            const iconColor = isLevel3 ? 'var(--warning, #eab308)' : 'var(--primary)';
            const parentName = isLevel3 ? (cas.find(p => p.id === ca.parent_id)?.name || 'Unknown') : 'Root CA';
            
            return `
            <div class="stat-card" style="display: flex; flex-direction: column; align-items: flex-start; gap: 1rem; position: relative; border: 1px solid var(--glass-border); border-left: 4px solid ${iconColor};">
                <div style="display: flex; align-items: center; gap: 15px; width: 100%;">
                    <div class="stat-icon" style="background: ${iconColor}22; color: ${iconColor};"><i class="fas fa-sitemap"></i></div>
                    <div style="flex-grow: 1;">
                        <h3 style="margin-bottom: 5px; font-size: 1.2rem;">${ca.name}</h3>
                        <div style="color: ${iconColor}; font-size: 0.85rem; font-weight: bold;">${levelLabel}</div>
                    </div>
                </div>
                <div style="width: 100%; border-top: 1px solid var(--glass-border); padding-top: 1rem; font-size: 0.9rem;">
                    ...
                    <div style="margin-top: 5px;"><strong style="color: var(--text-secondary);">Firmado por:</strong> ${parentName}</div>
    ...
    async function renderFederation() {
        viewTitle.innerText = 'Federación RCS (Yuxi)';
        const rcsStatus = await apiRequest('/yuxi/status') || { linked: false };
 
        contentArea.innerHTML = `
            <div class="federation-container">
                <div class="status-card" style="background: var(--glass-bg); padding: 2rem; border-radius: 24px; border: 1px solid var(--glass-border); margin-bottom: 2rem;">
                    <div style="display: flex; align-items: center; justify-content: space-between;">
                        <div style="display: flex; align-items: center; gap: 20px;">
                            <div class="stat-icon ${rcsStatus.linked ? 'green' : 'red'}" style="width: 60px; height: 60px; font-size: 1.5rem;">
                                <i class="fas ${rcsStatus.linked ? 'fa-link' : 'fa-link-slash'}"></i>
                            </div>
                            <div>
                                <h2 style="margin-bottom: 5px;">Estatus de la Red Sovereign</h2>
                                <p style="color: var(--text-secondary);">${rcsStatus.linked ? 'Tu Certberus está federado con la comunidad.' : 'Este nodo opera en modo privado (Stand-alone).'}</p>
                            </div>
                        </div>
                        <div class="badge" style="background: ${rcsStatus.linked ? 'var(--success)' : 'var(--danger)'}33; color: ${rcsStatus.linked ? 'var(--success)' : 'var(--danger)'};">
                            ${rcsStatus.linked ? 'FEDERADO' : 'SÓLO LOCAL'}
                        </div>
                    </div>
                </div>
 
                ${!rcsStatus.linked ? `
                <div class="config-card" style="background: rgba(255,165,0,0.05); padding: 2rem; border-radius: 24px; border: 1px solid rgba(255,165,0,0.2);">
                    <h3>Unirse a la Federación</h3>
                    <p style="margin-bottom: 1.5rem; color: var(--text-secondary);">Para que tus certificados sean reconocidos por el Root CA comunitario y otros navegadores Brave/Web3, debes solicitar unirte a la red.</p>
                    <ol style="margin-left: 1.5rem; display: grid; gap: 10px; color: var(--text-secondary);">
                        <li>Genera un <strong>CSR de Federación</strong> desde la CLI: <code style="background: #000; padding: 2px 6px; border-radius: 4px;">yuxi request &lt;slug&gt;</code></li>
                        <li>Envía la solicitud a través del Portal de Gobernanza RCS.</li>
                        <li>Una vez aprobado, activa el sello con: <code style="background: #000; padding: 2px 6px; border-radius: 4px;">yuxi activate &lt;slug&gt;</code></li>
                    </ol>
                </div>
                ` : `
                <div class="stats-grid" style="grid-template-columns: repeat(2, 1fr);">
                    <div class="stat-card">
                        <h3>Anclaje Blockchain</h3>
                        <p style="font-size: 0.85rem; color: var(--text-secondary);">Transacciones verificadas en el ledger.</p>
                        <div style="margin-top: 1rem; color: var(--success); font-weight: 600;">
                            <i class="fas fa-check-circle"></i> Sincronizado
                        </div>
                    </div>
                    <div class="stat-card">
                        <h3>Gobernanza</h3>
                        <p style="font-size: 0.85rem; color: var(--text-secondary);">Estatus de tu CA intermedia.</p>
                        <div style="margin-top: 1rem; color: var(--success); font-weight: 600;">
                            <i class="fas fa-user-shield"></i> Reputación Alta
                        </div>
                    </div>
                </div>
                `}
    ...
    async function renderLogs() {
        ...
        const logRows = logs.map(log => {
            ...
            let summaryHTML = log.response_summary || '---';
            let extraDetails = '---';
            if (summaryHTML.includes('(Reason:')) {
                const match = summaryHTML.match(/\(Reason:\s*(.+?)\)/);
                if (match) {
                    extraDetails = `<span class="status-badge revoked" style="font-size: 0.70rem; padding: 2px 6px;"><i class="fas fa-info-circle"></i> ${match[1]}</span>`;
                    summaryHTML = summaryHTML.replace(match[0], '').trim();
                }
            }
            
            let shortEndpoint = log.endpoint;
            if (shortEndpoint.length > 30) {
                shortEndpoint = shortEndpoint.replace(/([a-fA-F0-9]{32,64})/g, (match) => match.substring(0, 8) + '...');
            }

            if (summaryHTML.length > 30) {
                summaryHTML = summaryHTML.replace(/([a-fA-F0-9]{32,64})/g, (match) => match.substring(0, 8) + '...');
            }

            const shortSerial = log.serial_number ? log.serial_number.substring(0, 8) + '...' : '---';
            
            return `
                <tr>
                    <td style="font-size: 0.8rem; color: var(--text-secondary); width: 120px;">${date}</td>
                    <td><span class="method-badge ${methodClass}">${log.method}</span></td>
                    <td style="font-family: monospace; font-size: 0.85rem;" title="${log.endpoint}">${shortEndpoint}</td>
                    <td class="status-text ${statusClass}">${log.status_code}</td>
                    <td><span class="badge" style="font-size: 0.75rem;">${log.token_type}</span></td>
                    <td class="log-details-cell" title="${log.response_summary || ''}">${summaryHTML}</td>
                    <td style="font-family: monospace; font-size: 0.8rem;" title="${log.serial_number || ''}">${shortSerial}</td>
                    <td>${extraDetails}</td>
                </tr>
            `;
        }).join('');
        ...
        contentArea.innerHTML = `
            <div class="data-table-container">
                ...
                <table>
                    ...
                    <tbody>
                        ${logRows.length ? logRows : '<tr><td colspan="8" style="text-align: center; color: var(--text-secondary); padding: 2rem;">No hay registros de auditoría aún.</td></tr>'}
                    </tbody>
                </table>
            </div>
        `;
