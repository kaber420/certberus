...
async def revoke_certificate(serial: str, request: RevokeRequest):
    async with db_session.AsyncSessionLocal() as session:
        ...
        try:
            auth_name = "default"
            if cert.authority_id:
                auth_result = await session.execute(select(Authority).where(Authority.id == cert.authority_id))
                auth = auth_result.scalars().first()
                if auth:
                    auth_name = auth.name
                    
            revoked_query = select(Certificate).where(Certificate.revoked_at != None)
            if cert.authority_id:
                revoked_query = revoked_query.where(Certificate.authority_id == cert.authority_id)
            else:
                revoked_query = revoked_query.where(Certificate.authority_id == None)
                
            revoked_result = await session.execute(revoked_query)
            ...
            crl_pem = pki.generate_crl(revoked_meta, ca_password=pwd, days=7, authority_name=auth_name if auth_name != "default" else None)
            crl_name = f"crl_{auth_name}.pem" if auth_name != "default" else "crl.pem"
            
            with open(pki.storage_path / crl_name, "wb") as f:
                f.write(crl_pem)
        except Exception as e:
            import logging
            logging.error(f"Failed to auto-generate CRL during revocation: {e}")
        ...
        await log_event(
            ...
            response_summary=f"Certificate {serial} revoked (Reason: {request.reason})",
